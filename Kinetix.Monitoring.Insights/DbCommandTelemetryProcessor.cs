...
public class DbCommandTelemetryProcessor : ITelemetryProcessor
{
    ...
    {
        if (
            item is TraceTelemetry trace
            && trace.Properties.TryGetValue("CategoryName", out string categoryName)
        ...
        )
        {
            ...
            var queryName =
                queryMatch.Groups.Count == 4 ? $"{queryMatch.Groups[1].Value} {queryMatch.Groups[3].Value}" : "query";
