...
namespace Kinetix.Caching.Store
{
    ...
    internal sealed class DiskStore : IStore, IDisposable
    {
        ...
        public DiskStore(ILogger<CacheManager> logger, IEhcache cache, string diskPath)
        {
            _logger = logger;
            this._cache = cache;
        ...
        public void Dispose()
        {
            ...
            try
            {
                ...
                if (!_persistent)
                {
                    _logger.LogDebug("Deleting file " + _dataFile.Name);
                    _dataFile.Delete();
                }
            }
            catch (IOException e)
        ...
        public Element Get(object key)
        {
            lock (this)
            {
                try
                {
                    ...
                    if (!_diskElements.TryGetValue(key, out DiskElement diskElement))
                    {
                        return null;
                    }
        ...
        public void Put(Element element)
        {
            try
            {
                ...
                {
                    _logger.LogError(_name + "Cache: Elements cannot be written to disk store because the" +
        ...
        public void RemoveAll()
        {
            lock (this)
            {
                ...
                {
                    _logger.LogError(_name + " Cache: Could not rebuild disk store. Initial cause was " + e.Message, e);
                    this.Dispose();
                }
            }
        }
        ...
        internal bool BackedUp()
        {
            ...
            if (backedUp)
            {
                _logger.LogDebug("A back up on cache puts occurred. Consider increasing diskSpoolBufferSizeMB for cache " + _name);
            }
        ...
        private DiskElement CheckForFreeBlock(int bufferLength)
        {
            ...
            if (diskElement == null)
            {
                ...
                {
                    Position = _randomAccessFile.Length,
        ...
        private void CreateNewIndexFile()
        {
            ...
            {
                _indexFile.Delete();
                _logger.LogDebug("Index file " + _indexFile.Name + " deleted.");
            }
            else
            {
                FileStream fs = null;
                try
                {
                    ...
                    _logger.LogDebug("Index file " + _indexFile.Name + " created successfully");
                }
                catch (IOException e)
                {
        ...
        private void DeleteIndexIfNoData()
        {
            ...
            {
                _logger.LogDebug("Matching data file missing for index file. Deleting index file " + this.IndexFileName);
        ...
        private void ExpireElements()
        {
            ...
            try
            {
                lock (_spoolLock)
                {
                    ...
                    {
                        _logger.LogDebug(_name + "Cache: Removing expired spool element " + element.Key);
                ...
                lock (_diskElements)
                {
                    ...
                    {
                        _logger.LogDebug(_name + "Cache: Removing expired spool element " + diskElement.Key + " from Disk Store");
        ...
        private void InitialiseFiles()
        {
            ...
            if (_persistent)
            {
                if (!ReadIndex())
                {
                    _logger.LogDebug("Index file dirty or empty. Deleting data file " + this.DataFileName);
            ...
            {
                _logger.LogDebug("Deleting data file " + this.DataFileName);
            ...
            lock (this)
            {
                ...
                if (_indexFile.Exists)
                {
                    ...
                    catch (IOException ioe)
                    {
                        ...
                        _logger.LogDebug("IOException reading index. Creating new index. ", ioe);
                    }
                    catch (SerializationException se)
                    {
                        _logger.LogError("Corrupt index file. Creating new index.", se);
                    }
                    finally
                    {
                        if (indexStream != null)
        ...
        private byte[] SerializeEntry(Element element)
        {
            ...
            {
                _logger.LogError(_name + "Cache : Failed to serialize. Reason: " + e.Message, e);
            }
        ...
        private void WriteElement(Element element, object key)
        {
            try
            {
                ...
                {
                    _logger.LogError("OutOfMemoryError on serialize: " + key, me);
                }
            }
            catch (IOException e)
            {
