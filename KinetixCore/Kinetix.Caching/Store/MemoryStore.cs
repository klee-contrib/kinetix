...
namespace Kinetix.Caching.Store
{
    ...
    internal abstract class MemoryStore : IStore
    {
        ...
        protected MemoryStore(ILogger<CacheManager> logger, IEhcache cache, IStore diskStore)
        {
            _logger = logger;
            this._diskStore = diskStore;
            this.Cache = cache;

            _logger.LogDebug("Initialized " + this.GetType().Name + " for " + cache.Name);
        }
        ...
        public void Flush()
        {
            lock (this)
            {
                ...
                {
                    _logger.LogDebug(this.Cache.Name + " is persistent. Spooling " + this.Map.Count + " elements to the disk store.");
        ...
        public Element Remove(object key)
        {
            lock (this)
            {
                ...
                {
                    _logger.LogDebug(this.Cache.Name + "Cache: Cannot remove entry as key " + key + " was not found");
                }
        ...
        internal static MemoryStore Create(ILogger<CacheManager> logger, IEhcache cache, IStore diskStore)
        {
            ...
            switch (policy)
            {
                case MemoryStoreEvictionPolicy.Lru:
                    memoryStore = new LruMemoryStore(logger, cache, diskStore);
                    break;
                default:
        ...
        internal void Evict(Element element)
        {
            ...
            if (this.Cache.IsOverflowToDisk)
            {
                ...
                {
                    _logger.LogDebug("Object with key " + element.Key + " is not Serializable and cannot be overflowed to disk");
                }
                else
                {
        ...
        protected void SpoolAllToDisk()
        {
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                ...
                if (!element.IsSerializable)
                {
                    _logger.LogDebug("Object with key " + element.Key
        ...
        protected virtual void SpoolToDisk(Element element)
        {
            ...
            _logger.LogDebug(this.Cache.Name + "Cache: spool to disk done for: " + element.Key);
        }
        ...
        private Element GetInternal(object key, bool updateStatistics)
        {
            lock (this)
            {
                ...
                if (element != null)
                {
                    ...
                    _logger.LogDebug(this.Cache.Name + "Cache: " + this.Cache.Name + "MemoryStore hit for " + key);
                }
                else
                {
                    _logger.LogDebug(this.Cache.Name + "Cache: " + this.Cache.Name + "MemoryStore miss for " + key);
                }
