...
namespace Kinetix.Data.SqlClient
{
        ...
        where T : class, new()
    {
        ...
        public static IEnumerable<T> ParseCommand(IReadCommand cmd)
        {
            if (cmd == null)
            {
            ...
            if (cmd is SqlServerCommand command)
            {
        ...
        public static T ParseCommandForSingleObject(IReadCommand cmd, bool returnNullIfZeroRow = false)
        {
            if (cmd == null)
            {
                throw new ArgumentNullException("cmd");
